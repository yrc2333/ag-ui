...
function loadEvents() {
  try {
    ...
    console.log('✅ 事件配置加载成功');
    console.log(`   共 ${eventsData.events.length} 个事件`);
    return eventsData;
  } catch (error) {
...
app.get('/api/agent/run', async (req, res) => {
  ...
  try {
    ...
    res.write(`data: ${JSON.stringify({ type: 'run-error', message: error.message })}\n\n`);
    res.end();
  }
});
